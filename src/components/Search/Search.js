...
class Search extends Component {
  ...
  submitStudent = (e) => {
    ...
    if (this.state.parent_name || this.state.phone || this.state.national_id)
      searchData.parent_info = {};
    if (this.state.parent_name)
      searchData.parent_info.full_name = this.state.parent_name;
    if (this.state.phone) searchData.parent_info.phone = this.state.phone;
    if (this.state.national_id)
      searchData.parent_info.national_id = this.state.national_id;
  ...
  render() {
    ...
    if (loading === true && students === null) {
      ...
      tableContent = (
        <h1 className="display-4 text-danger">No Students Found :(</h1>
      );
    } else {
    ...
    return (
      <SidebarTemplate>
        ...
        <div className="mt-3">{tableContent}</div>
      </SidebarTemplate>
    );
  }
}
